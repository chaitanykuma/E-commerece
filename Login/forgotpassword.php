...
<head>
    ...
    <title>Forniture || Forgot Password</title>
</head>
<body>
<?php
if(isset($_POST["submit"]))
{
    ...
    if($row > 0)
    {
        ...
        $Time = date( 'd-m-Y h:i:s A', strtotime(' +10 minutes '));
        $expFormat = mktime(date("H"), date("i"), date("s"), date("m") ,date("d")+1, date("Y"));$expDate = $Time;
        $update = "UPDATE users set token='" . $token . "' ,exp_date='" . $expDate . "' WHERE email='" . $emailId . "'";
        mysqli_query($conn, $update);
        $link = "<a href='http://localhost/Furniture/Login/Passwordreset/passwordreset.php?key=".$emailId."&token=".$token."'>Reset Password</a>";
        $message = '<div>
        <p><b>Hello!</b></p>
        <p>You are recieving this email because we recieved a password reset request for your account.</p>
        <p>The link is valid for 10 minutes.</p><br>
        <br>
        <p><button class="btn btn-primary">'.$link.'</a></button></p>
        <br>
          ...
          <div class="card-body p-3 p-md-4 p-xl-5">
            ...
            <form method="POST" action="">
              <div class="row gy-3 overflow-hidden">
                ...
                <div class="col-12">
                  <div class="d-grid">
                    <button class="btn bsb-btn-xl btn-primary" name="submit" type="submit">Reset Password</button>
                  </div>
                </div>
              </div>
              <p class="error" style="color: #FF0000;"><?php if(!empty($err)){ echo "* ".$err; } ?></p>
            ...
            <div class="row">
              <div class="col-12">
                ...
                <div class="d-flex gap-2 gap-md-4 flex-column flex-md-row justify-content-md-end">
                  ...
                  <a href="../Signup/signup.php" class="link-secondary text-decoration-none">Register</a>
                </div>
              </div>
            </div>
            <div class="row">
